...
assert.match(source, /TextFieldValue\(text = query, selection = TextRange\(query\.length\)\)/);
...
assert.match(source, /if \(editable\) \{/);
assert.match(source, /readOnly = false/);
assert.match(source, /interactionSource = source,\n\s+indication = null/);
