...
assert.match(appBuild, /androidx\.compose:compose-bom:2026\.08\.00/);
...
assert.match(manifest, /android:name="\.ShowcaseActivity"/);
assert.match(manifest, /android:name="\.ShowcaseActivity"[\s\S]*?android:exported="true"[\s\S]*?android\.intent\.category\.LAUNCHER/);
assert.match(manifest, /android:name="\.MainActivity"\s+android:exported="false"\s*\/>/);
assert.equal((manifest.match(/android\.intent\.category\.LAUNCHER/g) ?? []).length, 1, "Android APK must expose exactly one launcher activity");
...
assert.doesNotMatch(source, /jan@example\.com/i, "Android reference fixtures must not contain personalized user data");
...
console.log("Compose Android reference application source/build/runtime contract tests passed with a single Showcase launcher, neutral fixtures, and Basic Checkbox/Radio/Select/Tabs/Tooltip/Toast/Progress/Slider/Navigation/Form Layout/Scroll Container/Menu coverage plus Phase 5 theme selection isolation.");
