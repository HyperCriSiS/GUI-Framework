...
function verifyButtons(palette, background) {
  ...
  const recipe = palette.themes.basic.components.button;
  ...
  for (const variant of contract.variants) {
    for (const state of ["default", "hover", "pressed"]) {
      ...
      assert.ok(foreground, `${palette.id} Basic ${variant} button needs a foreground`);
      assert.equal(root.opacity, undefined, `${palette.id} Basic ${variant} ${state} must not reduce enabled text contrast with whole-control opacity`);
      assertContrast(
        foreground,
        fill,
        MIN_TEXT_CONTRAST,
        `${palette.id} Basic ${variant} button ${state}`,
      );
    }
    ...
    assertContrast(
      ...
      `${palette.id} Basic ${variant} button focus outline`,
    );
  }
}

function verifyInput(palette, background) {
  const contract = palette.components.input;
  const recipe = palette.themes.basic.components.input;

  for (const state of ["default", "hover", "focus", "error"]) {
    const visual = resolve(recipe, contract, { variant: "standard", state });
    const root = visual.root;
    assertContrast(root.foreground, root.fill, MIN_TEXT_CONTRAST, `${palette.id} Basic input text ${state}`);
    assertContrast(visual.placeholder.foreground, root.fill, MIN_TEXT_CONTRAST, `${palette.id} Basic input placeholder ${state}`);
    assertContrast(root.border.color, root.fill, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic input inner boundary ${state}`);
    assertContrast(root.border.color, background, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic input outer boundary ${state}`);
  }
  ...
  assertContrast(
    ...
    background,
    MIN_NON_TEXT_CONTRAST,
    `${palette.id} Basic input focus outline`,
  );
}

function verifySwitch(palette, background) {
  const contract = palette.components.switch;
  const recipe = palette.themes.basic.components.switch;

  for (const state of ["default", "hover", "pressed"]) {
    const visual = resolve(recipe, contract, { variant: "standard", state });
    assertContrast(visual.root.border.color, visual.root.fill, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic switch inner boundary ${state}`);
    assertContrast(visual.root.border.color, background, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic switch outer boundary ${state}`);
  }

  const checked = resolve(recipe, contract, { variant: "standard", state: "checked" });
  assertContrast(checked.root.fill, background, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic switch checked track`);
  assertContrast(checked.thumb.fill, checked.root.fill, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic switch checked thumb`);

  const focused = resolve(recipe, contract, { variant: "standard", state: "focus" });
  assertContrast(focused.root.outline.color, background, MIN_NON_TEXT_CONTRAST, `${palette.id} Basic switch focus outline`);
  ...
  for (const size of contract.sizes) {
    ...
    assert.ok(
      ...
      `${palette.id} Basic switch ${size} target must be at least ${MIN_TARGET_SIZE_PX} by ${MIN_TARGET_SIZE_PX} CSS px`,
    );
  }
}
