...
function validName(name) {
  return name === "$root" || (!name.startsWith("$") && !/[{}.]/.test(name));
}
...
function collectTokens(node, file, path = [], inheritedType, tokens = new Map()) {
  ...
  for (const [name, value] of Object.entries(node)) {
    ...
    if (Object.prototype.hasOwnProperty.call(value, "$value")) {
      ...
      if (!type) {
        errors.push(`${file}: token ${childPath.join(".")} has no explicit or inherited $type`);
      } else if (!allowedTypes.has(type)) {
        errors.push(`${file}: token ${childPath.join(".")} uses unsupported type ${JSON.stringify(type)}`);
      }
...
function validateTokenValue(token, label, tokens) {
  ...
  if (isReference(value)) {
    ...
    if (!target) {
      errors.push(`${label}: unresolved token reference ${value}`);
    } else if (type && target.type && type !== target.type) {
      errors.push(`${label}: reference ${value} resolves to ${target.type}, expected ${type}`);
    }
    return;
  }
  ...
  switch (type) {
    ...
    case "dimension":
      ...
      break;
    case "duration":
      if (!value || typeof value !== "object" || typeof value.value !== "number" || !["ms", "s"].includes(value.unit)) {
        errors.push(`${label}: duration must contain numeric value and unit ms/s`);
      }
      break;
    case "cubicBezier":
      if (!Array.isArray(value) || value.length !== 4 || value.some((item) => typeof item !== "number") || value[0] < 0 || value[0] > 1 || value[2] < 0 || value[2] > 1) {
        errors.push(`${label}: cubicBezier must contain four numbers with x coordinates in the range 0..1`);
      }
      break;
    case "number":
      if (typeof value !== "number") errors.push(`${label}: number token must contain a JSON number`);
      break;
    case "transition": {
      if (!value || typeof value !== "object" || Array.isArray(value)) {
      ...
      for (const [field, expectedType] of fields) {
        ...
        } else if (expectedType === "duration") {
          validateTokenValue({ type: "duration", value: fieldValue }, `${label}.${field}`, tokens);
        } else {
          validateTokenValue({ type: "cubicBezier", value: fieldValue }, `${label}.${field}`, tokens);
        }
      }
      break;
    }
...
async function validateFile(file) {
  ...
  for (const [path, token] of tokens) {
    validateTokenValue(token, `${file}:${path}`, tokens);
  }
  return tokens;
}
...
for (const directory of tokenDirectories) {
  ...
  for (const entry of entries.filter((name) => name.endsWith(".tokens.json")).sort()) {
    await validateFile(join(directory, entry));
  }
}
