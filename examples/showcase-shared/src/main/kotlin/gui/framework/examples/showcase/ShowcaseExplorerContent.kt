...
@Composable
private fun ShowcaseText(text: String, paletteId: String, role: ShowcaseTextRole = ShowcaseTextRole.Body) {
    ...
    CompositionLocalProvider(LocalDensity provides qaDensity) {
        GuiTheme(theme = theme, paletteId = paletteId) {
            ...
            Box(
                ...
                val viewportModifier = viewport.maxWidthDp?.let { Modifier.fillMaxWidth().widthIn(max = it.dp) } ?: Modifier.fillMaxWidth()
                Column(
...
private fun ConfigurationPanel(
    theme: GuiThemeId, onThemeChange: (GuiThemeId) -> Unit,
    paletteId: String, onPaletteChange: (String) -> Unit,
    density: ShowcaseDensity, onDensityChange: (ShowcaseDensity) -> Unit,
    fontScale: Float, onFontScaleChange: (Float) -> Unit,
    viewport: ShowcaseViewport, onViewportChange: (ShowcaseViewport) -> Unit,
    themeExpanded: Boolean, onThemeExpandedChange: (Boolean) -> Unit,
    paletteExpanded: Boolean, onPaletteExpandedChange: (Boolean) -> Unit,
    densityExpanded: Boolean, onDensityExpandedChange: (Boolean) -> Unit,
    fontScaleExpanded: Boolean, onFontScaleExpandedChange: (Boolean) -> Unit,
    viewportExpanded: Boolean, onViewportExpandedChange: (Boolean) -> Unit,
    sizes: ShowcaseSizes,
) {
    GuiPanel(accessibilityLabel = "Showcase configuration", size = sizes.panel) {
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            GuiSelect(
                ...
                onValueChange = onPaletteChange, expanded = paletteExpanded, onExpandedChange = onPaletteExpandedChange,
                accessibilityLabel = "Palette", size = sizes.select,
            )
            GuiSelect(
                ...
                expanded = densityExpanded, onExpandedChange = onDensityExpandedChange,
                accessibilityLabel = "Density", size = sizes.select,
            )
            GuiSelect(
                ...
                onValueChange = { onFontScaleChange(it.toFloat()) }, expanded = fontScaleExpanded,
                onExpandedChange = onFontScaleExpandedChange, accessibilityLabel = "Font scale", size = sizes.select,
            )
            GuiSelect(
                ...
                expanded = viewportExpanded, onExpandedChange = onViewportExpandedChange,
                accessibilityLabel = "Viewport", size = sizes.select,
            )
        }
    }
}
...
private fun ComponentGallery(paletteId: String, sizes: ShowcaseSizes) {
    ...
    GuiTabs(
        ...
        onValueChange = { page = it }, accessibilityLabel = "Component gallery category", size = sizes.tabs,
...
private fun ControlComponents(paletteId: String, sizes: ShowcaseSizes) {
    ...
    var primaryActivations by remember { mutableStateOf(0) }

    GalleryPanel("Buttons", "Button states and interaction", paletteId, sizes.panel) {
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            GuiButton("Primary", onActivate = { primaryActivations += 1 }, size = sizes.button)
            GuiButton("Loading", onActivate = {}, loading = true, size = sizes.button)
            GuiButton("Disabled", onActivate = {}, disabled = true, size = sizes.button)
        }
        ShowcaseText("Primary activations: $primaryActivations", paletteId, ShowcaseTextRole.Muted)
    }
    GalleryPanel("Input & Select", "Editable and controlled choice states", paletteId, sizes.panel) {
        ...
        GuiSelect(
            ...
            onValueChange = { selected = it }, expanded = selectExpanded, onExpandedChange = { selectExpanded = it },
            accessibilityLabel = "Mode selection", size = sizes.select,
        )
    }
...
private fun DataComponents(paletteId: String, sizes: ShowcaseSizes) {
    ...
    GalleryPanel("Table", "Passive structured data", paletteId, sizes.panel) {
        GuiTable(
            ...
            caption = "Framework component status", accessibilityLabel = "Component status table", size = sizes.table,
        )
    }
    GalleryPanel("Data Grid", "Controlled row selection and activation", paletteId, sizes.panel) {
        GuiDataGrid(
            ...
            onValueChange = { gridValue = it }, onRowActivate = { gridValue = it }, accessibilityLabel = "Worker data grid", size = sizes.dataGrid,
        )
    ...
    GalleryPanel("Tree", "Expandable hierarchy and selection", paletteId, sizes.panel) {
        GuiTree(
            value = treeValue, items = sampleTree(expandedNodes), onValueChange = { treeValue = it },
            onExpandedChange = { value -> expandedNodes = if (value in expandedNodes) expandedNodes - value else expandedNodes + value },
            onNodeActivate = { treeValue = it }, accessibilityLabel = "Framework hierarchy", size = sizes.tree,
        )
...
private fun LayoutComponents(paletteId: String, sizes: ShowcaseSizes) {
    ...
    var formStatus by remember { mutableStateOf("Not saved") }
    GalleryPanel("Navigation & Tabs", "Controlled navigation primitives", paletteId, sizes.panel) {
        GuiNavigation(
            ...
            onValueChange = { navigationValue = it }, accessibilityLabel = "Gallery navigation", size = sizes.navigation,
        )
    ...
    GalleryPanel("Form Layout", "Responsive multi-column form with validation", paletteId, sizes.panel) {
        GuiFormLayout(columns = 2, accessibilityLabel = "Showcase form", size = sizes.form) {
            ...
            GuiFormActions {
                GuiButton("Save", onActivate = { formStatus = if (formEndpoint.startsWith("https://")) "Saved" else "Fix validation errors" }, size = sizes.button)
                GuiButton("Reset", onActivate = { formName = "Observatory"; formEndpoint = "https://api.example.invalid"; formStatus = "Reset" }, size = sizes.button)
            }
        }
        ShowcaseText("Form status: $formStatus", paletteId, ShowcaseTextRole.Muted)
    }
...
private fun FeedbackComponents(paletteId: String, sizes: ShowcaseSizes) {
    ...
    GalleryPanel("Menu & Tooltip", "Anchored popup interaction", paletteId, sizes.panel) {
        GuiMenu(
            ...
            onOpenChange = { menuOpen = it }, onActivate = { menuAction = it; menuOpen = false }, accessibilityLabel = "Showcase menu",
        ...
        GuiTooltip(
            open = tooltipOpen, content = "Framework tooltip — hover or focus the trigger", onOpenChange = { tooltipOpen = it },
            trigger = { source -> GuiButton("Tooltip target", onActivate = { tooltipOpen = !tooltipOpen }, interactionSource = source, size = sizes.button) },
        )
    }
    GalleryPanel("Dialog & Toast", "Controlled overlay and notification", paletteId, sizes.panel) {
        ...
        GuiToast(
            open = toastOpen, title = "Showcase notification", message = "The Toast component is rendered by the active framework theme.",
            actionLabel = "Dismiss", actionValue = "dismiss", durationMs = 0,
            onOpenChange = { toastOpen = it }, onActivate = { toastOpen = false },
        )
    }
}
...
private fun RealWorldScreens(paletteId: String, sizes: ShowcaseSizes) {
    ...
    GuiTabs(
        ...
        onValueChange = { screen = it }, accessibilityLabel = "Real-world screen selector", size = sizes.tabs,
    ) { selected -> ShowcaseText(selected.label, paletteId, ShowcaseTextRole.Muted) }
    when (screen) {
    ...
    GuiPanel(modifier = Modifier.fillMaxWidth(), accessibilityLabel = "Dashboard screen", size = sizes.panel) {
        Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            GuiNavigation(
                ...
                onValueChange = { nav = it }, accessibilityLabel = "Dashboard navigation", size = sizes.navigation,
            )
            ...
            GuiTable(
                ...
                rows = listOf(
                    GuiTableRow(listOf("Gateway", "Healthy", "18 ms")), GuiTableRow(listOf("Indexer", "Healthy", "41 ms")),
                    GuiTableRow(listOf("Telemetry", "Degraded", "126 ms")), GuiTableRow(listOf("Storage", "Healthy", "23 ms")),
                ),
                caption = "Live service overview", accessibilityLabel = "Live service overview", size = sizes.table,
            )
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            BoxWithConstraints(Modifier.fillMaxWidth()) {
                val tree: @Composable () -> Unit = {
                    GuiTree(
                        ...
                        onNodeActivate = { selectedTree = it }, accessibilityLabel = "Data source tree", size = sizes.tree,
                    )
                }
                val grid: @Composable () -> Unit = {
                    GuiDataGrid(
                        ...
                        onValueChange = { selectedRow = it }, onRowActivate = { selectedRow = it }, accessibilityLabel = "Event data grid", size = sizes.dataGrid,
                    )
                }
                if (maxWidth >= 820.dp) {
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                } else Column(verticalArrangement = Arrangement.spacedBy(12.dp)) { tree(); grid() }
            }
...
private fun SettingsScreen(paletteId: String, sizes: ShowcaseSizes) {
    ...
    var saveStatus by remember { mutableStateOf("No pending save") }
            ...
            GuiFormLayout(columns = 2, accessibilityLabel = "Application settings", size = sizes.form) {
                ...
                GuiFormField(label = "Update channel") {
                    GuiSelect(
                        ...
                        onValueChange = { channel = it }, expanded = channelExpanded, onExpandedChange = { channelExpanded = it },
                        accessibilityLabel = "Update channel", size = sizes.select,
                    )
                }
                ...
                GuiFormActions {
                    GuiButton("Save settings", onActivate = { saveStatus = if (cacheSize.toIntOrNull() != null) "Settings saved" else "Fix validation errors" }, size = sizes.button)
                    GuiButton("Restore defaults", onActivate = { endpoint = "https://example.invalid"; cacheSize = "512"; diagnostics = false; channel = "stable"; saveStatus = "Defaults restored" }, size = sizes.button)
                }
            }
            ShowcaseText(saveStatus, paletteId, ShowcaseTextRole.Muted)
        }
    }
}
...
private fun ComparisonCard(label: String, theme: GuiThemeId, paletteId: String, sizes: ShowcaseSizes, modifier: Modifier = Modifier) {
    GuiTheme(theme = theme, paletteId = paletteId) {
        ...
        var actionCount by remember(theme) { mutableStateOf(0) }
        GuiPanel(modifier = modifier.fillMaxWidth(), accessibilityLabel = "Theme comparison $label", size = sizes.panel) {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                GuiButton("Primary action", onActivate = { actionCount += 1 }, size = sizes.button)
                ShowcaseText("Actions: $actionCount", paletteId, ShowcaseTextRole.Muted)
                GuiButton("Disabled", onActivate = {}, disabled = true, size = sizes.button)
            }
        }
    }
...
private fun StressLab(paletteId: String, sizes: ShowcaseSizes, defaultControlCount: Int) {
    ...
    var lastControlActivation by remember { mutableStateOf("None") }
    ...
    GuiPanel(modifier = Modifier.fillMaxWidth(), accessibilityLabel = "Stress Lab controls", size = sizes.panel) {
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            GuiTabs(
                ...
                onValueChange = { mode = it }, accessibilityLabel = "Stress scenario", size = sizes.tabs,
            ) { selected -> ShowcaseText(selected.label, paletteId, ShowcaseTextRole.Muted) }
            GuiSelect(
                value = workload,
                ...
                onValueChange = { workload = it }, expanded = workloadExpanded, onExpandedChange = { workloadExpanded = it },
                accessibilityLabel = "Stress workload", size = sizes.select,
            )
    ...
    when (mode) {
        "grid" -> GuiDataGrid(
            ...
            onValueChange = { gridValue = it }, onRowActivate = { gridValue = it }, accessibilityLabel = "Large stress data grid", size = sizes.dataGrid,
        )
        "tree" -> GuiTree(
            value = treeValue, items = stressTree(controlCount, expanded), onValueChange = { treeValue = it },
            onExpandedChange = { value -> expanded = if (value in expanded) expanded - value else expanded + value },
            onNodeActivate = { treeValue = it }, accessibilityLabel = "Large stress tree", size = sizes.tree,
        )
        "text" -> GuiPanel(modifier = Modifier.fillMaxWidth(), accessibilityLabel = "Text stress", size = sizes.panel) {
            Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                repeat(controlCount.coerceAtMost(500)) { index -> ShowcaseText("Row ${index + 1}: A deliberately long text sample — ÄÖÜ äöü ß — 0123456789 — responsive layout robustness.", paletteId) }
            }
        }
            ...
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                ShowcaseText("Last activation: $lastControlActivation", paletteId, ShowcaseTextRole.Muted)
                repeat(controlCount) { index ->
                    val label = "Stress control ${index + 1}"
                    GuiButton(label, onActivate = { lastControlActivation = label }, modifier = Modifier.fillMaxWidth(), size = sizes.button)
                }
            }
        }
    }
