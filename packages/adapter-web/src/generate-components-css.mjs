...
function emitFoundation(lines, themeIds) {
  ...
  lines.push(
    ...
    `${scope} .gui-progress:where([data-gui-variant="linear"]):where([data-gui-state~="indeterminate"]) .gui-progress__indicator { inline-size: 40% !important; animation: gui-progress-linear-indeterminate 1.2s linear infinite; }`, "",
    ...
    `${scope} .gui-progress:where([data-gui-variant="circular"]):where([data-gui-state~="indeterminate"]) .gui-progress__indicator { animation: gui-progress-circular-indeterminate 1.2s linear infinite; }`, "",
...
function generate(ir) {
  ...
  lines.push("@media (prefers-reduced-motion: reduce) {", `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-button,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-checkbox,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-input,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-select,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-tabs,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-menu,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-toast,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-progress,`, `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-switch {`, "    transition-duration: 0ms !important;", "    transition-delay: 0ms !important;", "  }", `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-progress__indicator {`, "    animation: none !important;", "  }", `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-progress:where([data-gui-variant="linear"]):where([data-gui-state~="indeterminate"]) .gui-progress__indicator {`, "    transform: translateX(75%);", "  }", `  :where(${availableThemeIds.map((id) => `[data-gui-theme="${id}"]`).join(", ")}) .gui-progress:where([data-gui-variant="circular"]):where([data-gui-state~="indeterminate"]) .gui-progress__indicator {`, "    transform: rotate(-90deg);", "  }", "}", "");
  return `${lines.join("\n")}\n`;
}
