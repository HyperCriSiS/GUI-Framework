...
function cssTokenValue(token, label) {
  switch (token.type) {
    case "color":
      return colorValue(token.value, label);
    case "dimension":
    case "duration":
      return scalarWithUnit(token.value, label);
    case "number":
    ...
    case "transition": {
      ...
      if (!Array.isArray(value.timingFunction) || value.timingFunction.length !== 4) {
        throw new Error(`${label}.timingFunction: expected cubicBezier array`);
      }
      return `${duration} cubic-bezier(${value.timingFunction.join(", ")}) ${delay}`;
    }
    default:
      throw new Error(`${label}: token type ${token.type} has no Web CSS mapping yet`);
  }
}
...
function declarations(tokens, predicate) {
  return Object.entries(tokens)
    ...
    .map(([path, token]) => `  ${cssName(path)}: ${cssTokenValue(token, path)};`);
}

function assertSamePrimitiveSet(palettes) {
  if (palettes.length < 2) return;
  const reference = Object.entries(palettes[0].tokens)
    .filter(([path]) => !path.startsWith("semantic."))
    .map(([path, token]) => [path, token.type, JSON.stringify(token.value)]);

  for (const palette of palettes.slice(1)) {
    const current = Object.entries(palette.tokens)
      .filter(([path]) => !path.startsWith("semantic."))
      .map(([path, token]) => [path, token.type, JSON.stringify(token.value)]);
    if (JSON.stringify(reference) !== JSON.stringify(current)) {
      throw new Error(`Primitive token set differs for palette ${palette.id}; primitives must remain palette-independent`);
    }
  }
}
...
function generate(ir) {
  ...
  const output = [
    ...
    "",
    ":root {",
    ...declarations(ir.palettes[0].tokens, (path) => !path.startsWith("semantic.")),
    "}",
    ""
  ];
